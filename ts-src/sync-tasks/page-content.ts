...
import { CmsPage, MultiSitePage, MultiSitePageMap } from '../lib/multi-site-page'

function cloneDocument(page: Document) {
  let { id, insertDate, updateDate, contentUpdateDate, ...rest } = page
  return rest
}
function tranformNavigationNode(multiSitePage: MultiSitePage, sourceTargetIdMap: any) {
  ...
  if (targetNode?.ParentId.startsWith('page')) {
    ...
    if (!targetId) {
      console.log(`No target id while trying to create new navigation node: ${sourceId}`)
      return
    }
...
export class SyncPageContent {
  ...
  async syncTemplatesWithoutPages() {
    const sourceTemplates = this.sourceDocs.get(DOCUMENT_LISTS.PAGE_TEMPLATE_CONTENT) as Document[]
  ...
  private async buildTargetNavigationData() {
    const sourceTargetIdMap = MultiSitePageMap.sourceToTargetPageIdMap(this.pageMap)
    ...
    for (const [name, multiSitePage] of this.pageMap.entries()) {
      ...
      const newNavigationNode = tranformNavigationNode(multiSitePage, sourceTargetIdMap)
  ...
  async syncPage(multiSitePage: MultiSitePage) {
    ...
    if (multiSitePage.source?.content) {
      const targetContent = await this.createOrUpdateTargetContent(multiSitePage)
  ...
  async createOrUpdateTargetContent(multiSitePage: MultiSitePage) {
    ...
    if (!targetDocument) {
      targetDocument = cloneDocument(multiSitePage?.source?.content as Document)
    }
  ...
  private async updateDocument(targetDocument: Document) {
    try {
      ...
      this.results.errors.push(error)
      console.error(`Error updating document: ${error} target_name: ${targetDocument.name}`)
    }
  }
}
