...
export class SyncCatalogContent {
  ...
  async sync() {
    ...
    const categoryContent = catalogContent?.filter((doc) => doc.name?.startsWith('category'))
  ...
  async syncCategoryContent(categoryContent: Document[]) {
    for (const content of categoryContent) {
      ...
      if (!sourceCategoryId) {
        console.log('skipping, malfomed category content document name: ' + content.name)
        continue
      }

      let targetCategoryId = this.categoryMap.sourceTargetMap.get(sourceCategoryId)
      if (!targetCategoryId) {
        console.log(
          'skipping, could not find target category for source category: ' + sourceCategoryId
        )
        continue
      }
  ...
  async createOrUpdateTarget(targetCategoryId: number, sourceDocument: Document) {
    const existingDoc = await this.existingTargetDoc(targetCategoryId)
    let targetDocument: any = existingDoc
    if (!targetDocument) {
      targetDocument = cloneDocument(targetCategoryId, sourceDocument) as Document
    }
    if (!targetDocument) {
      targetDocument.properties = sourceDocument.properties
    }
    const transformedDocument = await transformPipeline(this.contentTransformers)(targetDocument)
