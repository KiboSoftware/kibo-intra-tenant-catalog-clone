...
export type RedirectTransformer = (sourceRedirect: Redirect) => Promise<Redirect>
...
function cloneRedirectDocument(sourceDoc: DocumentWithContent): DocumentWithContent {
  const { name, document } = sourceDoc

  const { id, contentLength, insertDate, updateDate, contentUpdateDate, ...rest } =
...
export class SyncRedirects {
  ...
  constructor(
    ...
    this.redirectTransformers = [defaultRedirectTransformer, ...(redirectTransformers || [])]
  }
  async sync() {
  ...
  async updateContent() {
    ...
    try {
      ...
      const response = await apiFetch(this.targetConfiguration, { path, method, body })
      ...
      console.error(`unable to update target document content: ${documentId} `, error)
    }
  }
