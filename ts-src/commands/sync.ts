...
async function redirects(sourceSite: Site, targetSite: Site) {
  const redirectTransformer: RedirectTransformer = async (sourceRedirect) => {
    ...
    if (d.includes(`/${sourceLocale}/`)) {
      sourceRedirect.d = sourceRedirect.d.replace(`/${sourceLocale}/`, `/${targetLocale}/`)
    }
...
async function catalogContent(sourceSite: Site, targetSite: Site) {
  const comparator: CategoryComparator = (source, target) => {
    ...
    if (categoryPrefix) {
      normalizedSource = source.categoryCode?.replace(`${categoryPrefix.toUpperCase()}-`, '')
    } else {
  ...
  const localeLinkTransformer: any = async (contentDocument: Document) => {
    ...
    try {
      ...
      const targetDZString = dzAsString.replaceAll(`/${sourceSitePrefix}/`, `/${targetSitePrefix}/`)
...
function printErrorSummary({
  ...
  redirectResult?.errors?.length && console.log(`Redirect Errors: ${redirectResult.errors.length}`)
  catalogContentResult?.errors?.length &&
    console.log(`Catalog Content Errors: ${catalogContentResult.errors.length}`)
  pageResult?.errors?.length && console.log(`Page Errors: ${pageResult.errors.length}`)
}
...
export async function sync(args: any) {
  ...
  printErrorSummary({ redirectResult, catalogContentResult, pageResult, themeSettingsResult })
  await dumpErrors(
