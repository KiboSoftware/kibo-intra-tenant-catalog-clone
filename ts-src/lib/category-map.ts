...
export type CategoryComparator = (source: Category, target: Category) => boolean
...
export class CategoryMap {
  ...
  async fetchCategories(api: CategoriesApi, startIndex: number, pageSize: number): Promise<any[]> {
    let data: any[] = []
    try {
      const categoryCollection = await api.getCategories({ startIndex, pageSize })
  ...
  async fetchAllCategories(
    ...
    return this.fetchAllCategories(api, startIndex + pageSize, pageSize, joinedData)
  }
  ...
  async build() {
    ...
    try {
      ...
      for (const sourceCategory of sourceCategories) {
        const targets = targetCategories.filter((c) => this.comparator(sourceCategory, c))
        if (targets.length > 1) {
          console.log(`Found multiple target categories for ${sourceCategory.categoryCode}`)
          continue
        }
        const targetCategory = targets[0]
        if (targetCategory) {
          this.sourceTargetMap.set(sourceCategory.id as number, targetCategory.id as number)
        } else {
          console.log(`Could not find target category for ${sourceCategory.categoryCode}`)
        }
      }
    } catch (error) {
