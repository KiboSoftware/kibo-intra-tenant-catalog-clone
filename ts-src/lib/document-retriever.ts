...
class DocumentRetriever {
  ...
  async fetchDocuments(
    ...
    let data: any[] = []
    try {
      ActionTracker.addAction(`fetch-${documentListName}-${startIndex}`, 'FETCH', documentListName)
      ...
      ActionTracker.markFailed(`fetch-${documentListName}-${startIndex}`, error as any)
    }
    return data
  }
  async fetchDocumentList(
    ...
    const data = await this.fetchDocuments(documentListName, startIndex, pageSize)
    const joinedData = [...collected, ...data]
    if (data.length < pageSize) {
      return joinedData
    }
    return this.fetchDocumentList(documentListName, startIndex + pageSize, pageSize, joinedData)
  }
  async findDocument(documentListName: string, documentName: string): Promise<any> {
    try {
      const documentsRequest: DocumentsApiGetDocumentsRequest = {
        documentListName,
  ...
  async writeJSONFile(data: any, listName: string, documentName?: string): Promise<void> {
    ...
    try {
      ...
      ActionTracker.addAction(`write-${listName}`, 'write', listName, { filePath })
  ...
  async load(documentLists: string[], persist: boolean = false): Promise<Map<string, Document[]>> {
  ...
  async fetchDocumentContent(document: Document): Promise<any> {
    ...
    const path = `/content/documentlists/{documentListName}/documents/{documentId}/content`
      .replace(`{${'documentListName'}}`, encodeURIComponent(String(documentListName)))
      .replace(`{${'documentId'}}`, encodeURIComponent(String(documentId)))

    try {
      const response = await apiFetch(this.targetConfiguration, { path, method: 'GET' })
