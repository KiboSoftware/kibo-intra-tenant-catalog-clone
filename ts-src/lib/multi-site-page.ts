...
export class CmsPage {
  ...
  static fromDocs(name: string, documentCollection: Map<string, Document[]>) {
    try {
      ...
      page = documentCollection.get(DOCUMENT_LISTS.PAGES)?.find((doc) => doc.name === name)
      ...
      if (page) {
        navigation = documentCollection
          ...
          ?.properties?.data.find((node: any) => node.Id === 'page^^pages@mozu^^' + page?.id)
      }
...
export class MultiSitePage {
  constructor(public name: string, public source?: CmsPage, public target?: CmsPage) {}
  static fromDocs(
    name: string,
...
export class MultiSitePageMap {
  static sourceToTargetPageIdMap(pageMap: Map<string, MultiSitePage>) {
    return Array.from(pageMap.values())
      .filter((multiSitePage) => multiSitePage?.source?.page && multiSitePage?.target?.page)
      .reduce((map, multiSitePage) => {
        map.set(multiSitePage.source?.page?.id as string, multiSitePage.target?.page?.id as string)
        return map
      }, new Map())
  }
}
