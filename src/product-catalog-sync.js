...
dotenv.config();
const productCatalogFile = '/Users/thomas.phipps/git/ps/kibo-intra-tenant-clone/testdata/products/kibo-export-catalog-2023-08-26T00_42_16/productcatalog.csv';
...
function createProductCatalogUpdateFile(catalogMapping, productCatalogFile, outputFile) {
    ...
    const destinationCatalog = productCatalogDict[catalogMapping.destination.name]||{};
    ...
    for (const productCode in sourceCatalog) {
        ...
        } else if (sourceProduct.CategoryCodes !== destinationProduct.CategoryCodes) {
            destinationProduct.CategoryCodes = sourceProduct.CategoryCodes;
            productCatalogUpdates.push(destinationProduct);
        }
    }

    const productCatalogUpdatesCsv = stringify(productCatalogUpdates, { header: true });
